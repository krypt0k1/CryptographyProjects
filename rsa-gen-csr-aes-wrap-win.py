...
def extract_appname_ident(key_label):
    ...
    try:
        ...
        if os_type == 'win32' or os_type == 'cygwin':
            ...
            # Set path to OpenSSL then run the command
...
if save_to_file == 'y':
    file_path = input("Enter the file name or full path to save the wrapped key (e.g C:\Temp or C:\Program Files\. Default is current working directory):").strip()
...
if csr_bool != 'y':
    ...
    command = f'openssl req -new -keyform engine -engine nfkm -key "{appname_ident}" -out {PKCS11_KEY_LABEL}_csr.pem'
...
if save_to_file == 'y':
    file_path = input("Enter the full path location to save the CSR (e.g C:\Temp\ or C:\Program Files\ default is current working directory): ").strip()
