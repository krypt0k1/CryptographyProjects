...
# Description
# This script can be utilized to import x.509 certificates to an Entrust(nCipher) nShield Hardware Security Module with CKA_TRUSTED. 
...
# 5. nCipher HSM's have three types of protection methods Module (accelerator slot), Operator Card Set (OCS), and Softcards (virtual tokens). 

# The /opt/nfast/cknfastrc (Linux) or C:\Program Files\nCipher\nfast\cknfastrc\ (Windows) file will require modifications depending on the desired protection method you want to use. 
...
# 3. Close the cmd prompt after confirming the certificate is imported and trusted using cklist.
...
LIB = 'C:\\Program Files\\nCipher\\nfast\\toolkits\\pkcs11\\cknfast.dll' # Windows library. Use /opt/nfast/toolkits/pkcs11/libcknfast.so for Linux.
lib = pkcs11.lib(LIB)
...
try:
    cert_obj = session.get_key(object_class=pkcs11.ObjectClass.CERTIFICATE, label='<certificate_label>') # Define the certificate label.
    if cert_obj:
...
cert_obj = session.create_object({pkcs11.Attribute.CLASS: pkcs11.ObjectClass.CERTIFICATE,
                                  ...
                                  pkcs11.Attribute.LABEL: '<certificate_label>', # Define the certificate label.
...
print('Imported Certificate label:', cert_obj.__getitem__(pkcs11.Attribute.LABEL))
print('Imported Certificate type:', cert_obj.__getitem__(pkcs11.Attribute.CERTIFICATE_TYPE))
