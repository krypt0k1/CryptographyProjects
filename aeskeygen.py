...
def parse_args():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                                     ...
                                     usage="%(prog)s [--generate]\n"
                                           "%(prog)s [--generate] --label default_key_label --key-size 256 --token-label 'loadshared accelerator' --label new_aes_256_key --attr ENCRYPT=false DECRYPT=false WRAP=true UNWRAP=true\n"
    ...
    parser.add_argument('-c', '--curve-name',
                        ...
                        required=False,
                        type=str,
                        choices=['secp256r1', 'secp384r1', 'secp521r1', 'secp256k1', 'brainpoolP256r1'],
                        default=False)
    parser.add_argument("-ft", "--find-token",
                        help="find the token with the given label",
                        required=False,
                        action="store_true",
                        default=False)
    parser.add_argument("-ls", "--list-slots",
                        help="list all available slots",
                        required=False,
                        action="store_true",
                        default=False)
...
def main():
    ...
    find_token = args["find_token"]
    pin = args["pin"]
    slot_label = args["list_slots"]
    attribute = args["attribute"]
    copy = args["copy"]
    #algorithm = args["algorithm"]
    ## Work in progress under keygen.py##
...
def delete_key(token_label, key_label, pin):
    try:
        ...
        with token.open(rw=True, user_pin=pin) as session:
            ...
            key.destroy()  
            console = Console()
            table = Table(show_header=True, header_style="bold red", show_lines=True, title="Key Deleted :wasteb ", title_style="Bold", border_style="green", style="bright", width=50)
...
def key_copy(token_label, copy, key_label, new_label, pin):
    try:
        ...
        with token.open(rw=True, user_pin=pin) as session:
            ...
            console = Console()
            table = Table(show_header=True, header_style="bold red", show_lines=True, title="Key Copied Succesfully:", title_style="italic", border_style="green", style="bright", width=100)
            table.add_column("Token Label")
