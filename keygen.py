...
from pkcs11.util.ec import encode_ec_public_key, encode_named_curve_parameters, encode_ecdsa_signature, decode_ecdsa_signature, decode_ec_public_key, decode_ec_private_key
from pkcs11.util.rsa import encode_rsa_public_key, decode_rsa_public_key, decode_rsa_private_key
from pkcs11.util import dh as dh_util
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                                     ...
                                     allow_abbrev=True,
                                     add_help=True,
                                     epilog = " Supports AES, RSA, ECDSA key generation, deletion, and copying. Plan to add support for other algorithms in the future.")
    ...
    parser.add_argument("-alg", "--algorithm",
                        ...
                        required=False,
                        type=str,
                        choices={"AES", "RSA", "ECDSA", "SHA256", "SHA512", "SHA1", "SHA3", "MD5", "HMAC", "DES",
...
# Call the appropriate function based on the arguments
    if args["find_token"]:
        ...
        gen_key(token_label, key_label, key_size, aes_template, private_rsa_key_template, public_rsa_key_template, algorithm, pin)
    elif args["delete"]:
...
ALGORITHM_MAP = {
    ...
    "ECDSA": {"key_type": pkcs11.KeyType.EC, },  # Specify the curve size or name if needed
    # Add more mappings as necessary
}


def gen_key(token_label, key_label, key_size, aes_template, private_rsa_key_template, public_rsa_key_template, algorithm, pin):
    try:
...
# Open a session. 
        ...
        with token.open(rw=True, user_pin=pin) as session:
            ...
            if algorithm == "AES":
                ...
                # For RSA, we need to generate a public and private key pair
                ...
                # For ECDSA, we need to generate a public and private key pair
                ...
                print_ecdsa_key_info(public_key, private_key)
...
def print_ecdsa_key_info(public, private):
    public_info = {#  Not allowed in public key
        "LABEL": public.__getitem__(pkcs11.Attribute.LABEL),
        "TOKEN": public.__getitem__(pkcs11.Attribute.TOKEN),
        "KEY TYPE": public.__getitem__(pkcs11.Attribute.KEY_TYPE),
...
def key_copy(token_label, key_label, new_label, pin):
    try:
        ...
        sys.exit(f"Multiple keys found with label='{new_label}'.")
    except Exception as e:
